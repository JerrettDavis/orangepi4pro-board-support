...
`u-boot-sun60iw2p1.bin`, but complete Allwinner boot-package generation is not
solved here.
...
Those complete `.fex` package files are not generated by the wrapper. A safe
test plan should use a separate recovery/test SD or a verified package rebuild,
then keep the current NVMe and SD boot media recoverable.
