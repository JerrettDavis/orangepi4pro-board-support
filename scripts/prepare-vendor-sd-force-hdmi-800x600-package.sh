...
output=${OUTPUT:-/var/cache/orangepi4pro-images/build/boot-package-candidates/boot_package_vendor-sd-scriptfirst-hdmi-phandles-force-800x600.fex}
work_dir=$(mktemp -d)
trap 'rm -rf "$work_dir"' EXIT

usage() {
  cat <<'USAGE'
Prepare a vendor SD U-Boot package with script-first boot and forced HDMI mode.

Usage:
  scripts/prepare-vendor-sd-force-hdmi-800x600-package.sh [--vendor PACKAGE] [--output PACKAGE]
...
This is file-only. It:
  ...
  - adds force-output and force_timing to /soc/sunxi-drm/route/disp0_hdmi0
    using conservative 800x600@60 DMT timing;
...
route_node=/soc/sunxi-drm/route/disp0_hdmi0
force_node=$route_node/force_timing
...
fdtput -t x "$work_dir/u-boot.dtb" "$hdmi_node" hdmi_power1 "$(printf '0x%x' "$dldo2_phandle")"

fdtput "$work_dir/u-boot.dtb" "$route_node" force-output
fdtput -c "$work_dir/u-boot.dtb" "$force_node"
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" clock-frequency 0x2625a00
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" hactive 0x320
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" hfront-porch 0x28
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" hsync-len 0x80
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" hback-porch 0x58
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" vactive 0x258
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" vfront-porch 0x01
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" vsync-len 0x04
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" vback-porch 0x17
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" hsync-active 0x01
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" vsync-active 0x01
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" de-active 0x01
fdtput -t x "$work_dir/u-boot.dtb" "$force_node" pixelclk-active 0x00
...
test "$(fdtget "$work_dir/u-boot.dtb" "$route_node" force-output 2>/dev/null)" = "" \
  || {
    printf 'ERROR: force-output boolean property did not stick\n' >&2
    exit 1
  }
test "$(fdtget "$work_dir/u-boot.dtb" "$force_node" hactive)" = "800" \
  || {
    printf 'ERROR: force_timing hactive patch did not stick\n' >&2
    exit 1
  }
test "$(fdtget "$work_dir/u-boot.dtb" "$force_node" vactive)" = "600" \
  || {
    printf 'ERROR: force_timing vactive patch did not stick\n' >&2
    exit 1
  }

python3 - "$work_dir/u-boot-scriptfirst.bin" "$work_dir/u-boot.dtb" "$dtb_offset" "$dtb_size" "$work_dir/u-boot-force-hdmi.bin" <<'PY'
from pathlib import Path
import sys
...
"$repo_root/scripts/sunxi-toc1-package.py" repack \
  ...
  --replace "u-boot=$work_dir/u-boot-force-hdmi.bin" \
  --output "$output"

"$repo_root/scripts/sunxi-toc1-package.py" inspect "$output"
printf '\nPatched embedded U-Boot DTB:\n'
printf '  hdmi_power0=%s\n' "$dcdc2_phandle"
printf '  hdmi_power1=%s\n' "$dldo2_phandle"
printf '  route=%s\n' "$route_node"
printf '  forced_mode=800x600@60\n'
printf '\nPrepared forced-HDMI package:\n'
sha256sum "$output"
